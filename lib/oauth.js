...
exports.refreshAccessToken = (config) => {
    return new Promise ((fulill, reject) => {
        let encodedClientCredentials = encode.base64(`${config.client.id}:${config.client.secret}`);
        ...
        fetch(config.uri.token, {
            ...
            body: `grant_type=refresh_token&refresh_token=${config.user.refresh}`
        }).catch((error) => {
            let refreshError = error.json();
            reject(refreshError);
        }).then((response) => {
            return response.json().catch((parseError) => {
