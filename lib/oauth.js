...
exports.getFitbitAccessToken = (authCode, appCredentials) => {
...
exports.getSlackAccessToken = (authCode, appCredentials) => {
    return new Promise ((fulfill, reject) => {
        let tokenRequestUri = `${appCredentials.token_uri}?client_id=${appCredentials.client_id}&client_secret=${appCredentials.client_secret}&code=${authCode}&redirect_uri=${encodeURI(appCredentials.redirect_uri)}`;
        console.log(`Token request: ${tokenRequestUri}`);

        fetch(tokenRequestUri, {
            method: 'GET',
        }).catch((fetchError) => {
            console.log(`Error fetching access token: ${fetchError}`);
        }).then((tokenRequestResponse) => {
            if (tokenRequestResponse.status >= 400) {
                reject(tokenRequestResponse.json());
            } else if (tokenRequestResponse.status < 400 && tokenRequestResponse.status >= 200) {
